...
sim_names = ["umQ00p0a", "umQ02p5a", "umQ04p5a", "umQ07p5a", "umQ10p0a", "umQ12p5a", "umQ15p0a", "umQ18p0a", "umQ22p0a"]

for i in range(len(sim_ages)):

    # read in Longhurst provinces shapefile
    lh_provs = gpd.read_file("Longhurst_world_{0}Ma.shp".format(sim_ages[i]))
    # read in cGENIE 2D biogem output
    biogem = ncdf.Dataset("cgenie_output/muffin.CB.{0}.config/biogem/fields_biogem_2d.nc".format(sim_names[i]), "r")
    ...
    cgenie_provs.to_excel("cgenie_provs_{0}.xlsx".format(sim_ages[i]))
